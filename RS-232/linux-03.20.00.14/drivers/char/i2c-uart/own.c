...
static unsigned short normal_i2c[]= {
        i2c_gpio_port->client,I2C_CLIENT_END
};
...
unsigned char xor_read(unsigned char chip_addr, unsigned char addr)
{
  int i;
...
unsigned int xor_write(unsigned char chip_addr, unsigned char addr, unsigned char value)
{
...
static int isr_thread(void * arg)
{
  	...
  	unsigned char  i =0;
        ...
        while(1)
                ...
                //printk("the LCR in thread  is %02x\n", xor_read(i2c_gpio_port->client, reg_info[XR20M1170REG_LCR][0]));

                int_val0 = xor_read(i2c_gpio_port->client, reg_info[XR20M1170REG_ISR][0]);
                //xor_write(i2c_gpio_port->client, reg_info[XR20M1170REG_IER][0],0x00);
        		...
        		case 0xc1:   
					 ...
					 i = xor_read(i2c_gpio_port->client, reg_info[XR20M1170REG_LSR][0]);
                        ...
                        default :  
                                        //wake_up_interruptible(&i2c_gpio_port->tx_wait);
                                //        printk("I am in ktread when int_val0 is  %02x\n",int_val0);
                                //        printk("the LSR is %02x\n", xor_read(i2c_gpio_port->client, reg_info[XR20M1170REG_LSR][0]));
                                        
                                break;
                                                                
                }
//        wake_up_interruptible(&i2c_gpio_port->tx_wait);
        //xor_write(i2c_gpio_port->client, reg_info[XR20M1170REG_IER][0],0x03);
                interruptible_sleep_on(&i2c_gpio_port->isr_wait); 
                goto retry;
...
retry:
      ...
      //  while(xor_read(i2c_gpio_port->client, reg_info[XR20M1170REG_LSR][0]) & 0x20)
		 while( ready_to_write() )
                {
                        temp = *(buf2 + count );
                   //     PRINTK_I2C_GPIO("com write %c\n",temp);        
...
/*EFR  */
	...
	//xor_write(client, reg_info[XR20M1170REG_EFR][0], 0xf0);
         temp = xor_read(client, reg_info[XR20M1170REG_EFR][0]);
...
/*FCR*/
        //xor_write(client, reg_info[XR20M1170REG_FCR][0], 0x01);
...
/*IOCRL*/
        ...
        //temp = xor_read(client, reg_info[XR20M1170REG_ISR][0]);
        
        //PRINTK_I2C_GPIO("i2c_gpio ISR data is :%02x\n",temp);
        //xor_write(client, reg_info[XR20M1170REG_IOINTENA][0], 0x00);
...
//        retval = request_irq(IRQ_GPIO_0,i2c_gpio_handler,SA_SHIRQ,"I2C_GPIO_DRV",i2c_gpio_port);
	retval = request_irq(irq_req, i2c_gpio_handler, IRQF_SHARED, "I2C_UART_DRV", i2c_gpio_port);
	 ...
	 i2c_gpio_init(client);   //problem with this function 
