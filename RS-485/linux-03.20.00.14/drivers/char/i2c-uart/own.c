...
static int isr_thread(void * arg)
{
	 ...
	 PRINTK_I2C_GPIO("i2c_gpio thread called scucessfully sreenivas \n");
		...
		switch(int_val0)
                {
                          ...
                          while(xor_read(i2c_gpio_port->client, reg_info[XR20M1170REG_LSR][0]) & 0x01)
                                ...
                                BUF_HEAD_RX = xor_read(i2c_gpio_port->client, reg_info[XR20M1170REG_RHR][0]);
						     	...
						     	read_timer.expires  = jiffies + HZ/100;
                                       ...
                                       //         printk("I am in interrupt case 0xc4 int_val0 is  %02x\n",int_val0);
                                        break;
                        ...
                        case 0xcc :
                                ...
                                //        printk("I am in interrupt case 0xcc int_val0 is  %02x\n",int_val0);
		...
		//			i = xor_read(i2c_gpio_port->client, reg_info[XR20M1170REG_MSR][0]);
                        default :  
        ...
        retry1:
           ...
           //     PRINTK_I2C_GPIO("I am here haha \n");
                ...
                //        PRINTK_I2C_GPIO("com read %02x\n",temp);
						...
						//printk("the received buffer of driver is %s",buf1);
...
//			PRINTK_I2C_GPIO("I think the error is here \n");
                        count += 1;
                        if(count == size )
                        {
                                goto out;
...
/*the problem is that isr_routine is not contunusuly running it gets halted some were in middle so the output is not getting ensure that function while loop must be contunusuly executing  */
