...
static int __devinit i2c_gpio_probe(struct i2c_client *client, const struct i2c_device_id *id,struct platform_device *pdev)
{
        int result ,err;
        int retval ;
	int irq_req;
...
/***************************************************************************/
	...
	err = register_chrdev(i2c_gpio_major, I2C_GPIO_DRIVER_NAME, &fops);
	        if (err)
	        {
...
//        retval = request_irq(IRQ_GPIO_0,i2c_gpio_handler,SA_SHIRQ,"I2C_GPIO_DRV",i2c_gpio_port);
	retval = request_irq(irq_req, i2c_gpio_handler, IRQF_TRIGGER_FALLING | IRQF_DISABLED | IRQF_SHARED, "I2C_UART_DRV", i2c_gpio_port);
        if(retval)
                {
